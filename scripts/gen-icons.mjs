...
const MAP = [
  ...
  ['GithubLogoIcon', 'github-logo', 'regular', 'Import from GitHub.'],
  ...
  ['DiscordIcon', 'discord-logo', 'fill', 'Community Discord, beside Report a bug. Phosphor carries the brand mark itself, so it stays on-set instead of being a lone hand-traced logo. FILL weight: the regular outline’s thin face features smear into a robot at 16px; the solid mark stays legible.'],
  ...
  ['InstructionsIcon', 'scroll', 'regular', 'Agent Instructions / system prompt — the panel-toolbar control. A scroll with text lines (the lucide scroll-text shape, on-set in Phosphor): the concept is the standing orders an agent reads. Deliberately NOT shared with the AGENTS.md tree glyph, which stays the robot — the file marks the agent contract, the control edits the orders.'],
  ['BotIcon', 'robot', 'regular', 'The AGENTS.md tree glyph — a vendor-neutral agent contract file; the robot marks the agent itself, distinct from the Instructions scroll.'],
  ['SquaresFourIcon', 'squares-four', 'regular', 'Templates — the sidebar entry and its gallery tab (the marketplace-gallery glyph).'],
  ['CaretUpDownIcon', 'caret-up-down', 'regular', 'The switcher mark (macOS/VS Code select idiom) — the sidebar folder header’s Change-folder trigger; distinct on purpose from the single fold chevron beside it.'],
