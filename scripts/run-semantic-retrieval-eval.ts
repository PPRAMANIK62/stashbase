...
const { stdout: commitOutput } = await promisify(execFile)('git', ['rev-parse', 'HEAD'], { cwd: repoRoot });
...
const result = await runSemanticRetrievalEval({
  ...
  async createBackend({ appDataRoot }): Promise<SemanticEvalBackend> {
    ...
    return {
      ...
      semanticSearch: async (query, topK, libraryRoot) => {
        const found = await retrieval.search({ mode: 'semantic', query, folderRoot: libraryRoot, topK });
        return found.evidence.map((item) => item.sourcePath);
      },
      exactSearch: async (query, topK, libraryRoot) => {
        const found = await retrieval.search({ mode: 'keyword', query, folderRoot: libraryRoot, topK });
        return found.evidence.map((item) => item.sourcePath);
      },
