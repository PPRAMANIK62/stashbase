...
for (const [source, markdown] of contents) {
  ...
  for (const match of markdown.matchAll(/`([^`\n]+)`/g)) {
    ...
    const hasRepoPrefix = /^(?:\.github|code-review|design-docs|docs|e2e|electron|mcp|native|python|release-checklists|scripts|server|shared|renderer|web-src)\//.test(referencedPath);
