          ...
          <AnimatePresence>
            {activeRect && (
              <motion.div
                ...
                aria-hidden
                className={cn('pointer-events-none absolute z-0 bg-hover', shape.container)}
                initial={{
                  opacity: 0,
                  top: activeRect.top,
        ...
        <div
          ...
          className={cn(
            ...
            inlineImage
              ...
              : isInline
                ? cn('flex flex-row items-center', compact ? 'gap-2.5 pl-3' : 'gap-3 pl-4')
        ...
        >
          ...
          {dismissible && (
            <button
              ...
              aria-label="Dismiss"
              className={cn(
                'absolute top-2 right-2 z-30 flex h-7 w-7 cursor-pointer items-center justify-center text-muted-foreground transition-colors duration-80 outline-none hover:text-foreground focus-visible:ring-1 focus-visible:ring-[color:var(--focus-ring,#6B97FF)]',
                ...
                // radius either way.
                hasImage ? 'bg-card/70 backdrop-blur-sm hover:bg-card' : 'hover:bg-hover',
...
function CardFeature({ icon: Icon, title, description }: CardFeatureProps) {
  ...
  const compact = sizeClasses.variant === 'compact';
  return (
    <div data-slot="card-feature" className={cn('flex items-start', sizeClasses.gap)}>
      {Icon && (
        <Icon
