...
export interface LazySurfaceOptions<Props> {
  /** Wraps the loaded surface so a failed chunk can be retried. */
  boundary?: (retry: () => void, children: ReactNode) => ReactNode;
...
export function lazySurface<Props extends object>(
  ...
  return function LazySurface(props: Props) {
    ...
    return boundary ? boundary(retry, content) : content;
  };
}
