...
export function replaceJsonNode(
  ...
  if (!analyzed.available) {
    throw new Error(`Replacement is not valid strict JSON: ${analyzed.message}`);
  }
...
export function deleteJsonPath(source: string, path: JsonPath): string {
  ...
  if (siblings.length > 1 && index < siblings.length - 1) {
    to = siblings[index + 1].offset;
  } else if (index > 0) {
    from = siblings[index - 1].offset + siblings[index - 1].length;
  }
...
export function addJsonChild(
  ...
  if (!valueAnalysis.available) {
    throw new Error(`New value is not valid strict JSON: ${valueAnalysis.message}`);
  }
  const located = locate(source, containerPath);
  const container = located.node;
  if (container.type !== 'array' && container.type !== 'object') {
    throw new Error('Values can only be added to objects or arrays.');
  }
  if (container.type === 'object') {
    ...
    if ((container.children ?? []).some((property) => property.children?.[0]?.value === key)) {
      throw new Error(`The key ${JSON.stringify(key)} already exists in this object.`);
    }
  }
...
export function reorderJsonArrayItem(
  ...
  if (
    ...
    throw new Error('Array item no longer exists.');
  }
  ...
  const raws = children
    .slice(low, high + 1)
    .map((child) =>
      source.slice(child.offset + located.bom, child.offset + located.bom + child.length),
    );
  ...
  for (let index = 0; index < separators.length; index += 1) {
    replacement += separators[index] + raws[index + 1];
  }
...
function locate(source: string, path: JsonPath): { bom: number; node: Node } {
  ...
  if (!root || errors.length) {
    throw new Error('The source is no longer valid JSON. Switch to Source mode.');
  }
  ...
  for (const part of path) {
    ...
    if (!node) throw new Error(`Path ${formatJsonPath(path)} no longer exists.`);
  }
  return { bom, node };
}
...
function unavailable(
  ...
  return { available: false, location, message, reason };
}
