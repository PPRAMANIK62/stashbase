...
| Documents and editing | J03 | Retain / [0002](../decisions/0002-feature-runtime-and-protocol-ownership.md), [0004](../decisions/0004-runtime-scope-shared-identities-and-navigation.md), [0006](../decisions/0006-rendering-and-resource-discipline.md), [0012](../decisions/0012-persistence-commands-and-contract-evidence.md), [0015](../decisions/0015-adopt-fluid-functionalism-base-ui.md) | Markdown Rendering; Document Viewers; File Transactions; Renderer Workspace | `SourceReference`; folder-scoped `DocumentTabsRuntime`; independently disposable `DocumentRuntime`; app-owned open-document workflow; versioned `DocumentSourceApi` query/save/overwrite adapter; document-owned live draft, save barrier, and conflict authority; Fluid document tabs, source, and conflict surfaces | Task 28's strict v1 session still receives only active-folder-relative tab identities; Tasks 30–33 reuse the legacy read/save authority through strict replacement adapters without changing its other consumers | Building | Tasks 29–33 evidence in [Subphase 3](subphases-3-documents.md) proves source/tab lifecycle, versioned Markdown/TXT loading and editing, expected-version autosave, byte-preserving reconciliation, navigation and native save barriers, paired conflict recovery, and exclusive reload/merge/overwrite decisions. Specialized editors/viewers and final J03 evidence remain Tasks 34–40 and finalization work. |
...
| Runtime contract validation | Building | Shared Zod schemas now cover the library folder dialog, uncredentialed renderer runtime configuration, correlated window context-release requests, library HTTP request/snapshot/failure envelopes, files listing/reveal and versioned direct-text source envelopes, and the bounded v1 workspace-session snapshot plus read/write responses. `pnpm test:protocols`, `pnpm test:electron-boundary`, server route/origin tests, renderer adapter tests, and the live `app://` smoke prove these entries; remaining HTTP, IPC, event, WebSocket, worker, and persisted protocols arrive with their owning slices. |
| Accessibility and keyboard operation | Building | Task 26 `FileTree` component evidence proves tree/treeitem semantics, one roving tab stop, collapsed-row omission, semantic levels/positions, and Up/Down/Home/End/Left/Right keyboard behavior against the replacement. Final axe, forced-colors, 200% scaling, platform screen-reader, and journey E2E evidence remains deferred to finalization. |
