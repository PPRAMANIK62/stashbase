...
| Library and folders | J02 | Retain / [0002](../decisions/0002-feature-runtime-and-protocol-ownership.md), [0004](../decisions/0004-runtime-scope-shared-identities-and-navigation.md), [0012](../decisions/0012-persistence-commands-and-contract-evidence.md) | Renderer Workspace; File Transactions; Data Lifecycle; Window Lifecycle | `LibrarySnapshot` query key `['library', 'membership']`; `WorkspaceRuntime` with immutable folder/generation scope, serializable tree state, and folder-owned query prefix; `FileTree` → `FilesApi`; validated library and files HTTP protocols; Electron-owned request authorization | First-folder registration is additive and uses existing server membership authority; file listing now has an additive strict response contract; legacy folder and file endpoints stay compatible until cutover | Building | Tasks 23–26 prove settled empty and populated no-active-folder welcome states, complete membership presentation, native open/create and cancellation, classified local failure/retry, selected-folder registration, authoritative active-folder replacement, subsequent-folder addition, main-owned request identity, folder-runtime creation/disposal, switch and runtime generations, scoped query cancellation, stale-completion rejection, one naturally ordered visible tree, semantic positions, roving keyboard navigation, restricted-entry reveal, generic-file disclosure, and bounded initial rendering. Exact evidence is listed in [Subphase 2](subphases-2-bootstrap-and-workspace.md#26--render-the-accessible-file-tree). Loss/removal and session restore remain Tasks 27–28; final J02 E2E, accessibility, performance, and visual evidence remains deferred. |
