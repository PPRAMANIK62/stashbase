...
| Documents and editing | J03 | Retain / [0002](../decisions/0002-feature-runtime-and-protocol-ownership.md), [0004](../decisions/0004-runtime-scope-shared-identities-and-navigation.md), [0006](../decisions/0006-rendering-and-resource-discipline.md), [0012](../decisions/0012-persistence-commands-and-contract-evidence.md), [0015](../decisions/0015-adopt-fluid-functionalism-base-ui.md) | Markdown Rendering; Document Viewers; File Transactions; Renderer Workspace | `SourceReference`; folder-scoped `DocumentTabsRuntime`; independently disposable `DocumentRuntime`; app-owned open-document workflow; versioned `DocumentSourceApi` query/save adapter; document-owned live draft and save authority; Fluid document tabs and source surface | Task 28's strict v1 session still receives only active-folder-relative tab identities; Tasks 30–31 reuse the legacy read/save routes through strict replacement adapters without changing their other consumers | Building | Tasks 29–31 evidence in [Subphase 3](subphases-3-documents.md) proves source/tab lifecycle, versioned Markdown/TXT loading and editing, explicit-folder reads and writes, read-only policy, expected-version autosave, byte-preserving no-ops, authoritative reconciliation, and recoverable save failures. Save barriers, conflict decisions, specialized viewers, and final J03 evidence remain Tasks 32–40 and finalization work. |
...
| Runtime contract validation | Building | Shared Zod schemas now cover the library folder dialog, uncredentialed renderer runtime configuration, library HTTP request/snapshot/failure envelopes, files listing/reveal and versioned direct-text source envelopes, and the bounded v1 workspace-session snapshot plus read/write responses. `pnpm test:protocols`, `pnpm test:electron-boundary`, server route/origin tests, renderer adapter tests, and the live `app://` smoke prove these entries; remaining HTTP, IPC, event, WebSocket, worker, and persisted protocols arrive with their owning slices. |
...
| Electron titlebar and window lifecycle | Building | `pnpm test:electron-boundary` and `env -u ELECTRON_RUN_AS_NODE pnpm test:electron-boundary:smoke` prove the replacement `app://` origin, production CSP, sandboxed typed preload, window/frame/origin/capability/payload authorization, default-deny window policy, active-folder registration, correlated release acknowledgements, authoritative removal notification, atomic workspace-session storage, first-window restore claiming, and reload-local snapshot retention. The current no-document workspace acknowledges release; Task 32 composes the editable-document save barrier into this typed seam. Full replacement lifecycle and final journey evidence remain deferred. |
