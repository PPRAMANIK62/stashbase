...
| Library and folders | J02 | Retain / [0002](../decisions/0002-feature-runtime-and-protocol-ownership.md), [0004](../decisions/0004-runtime-scope-shared-identities-and-navigation.md), [0012](../decisions/0012-persistence-commands-and-contract-evidence.md) | Renderer Workspace; File Transactions; Data Lifecycle; Window Lifecycle | `LibrarySnapshot` query key `['library', 'membership']`; `WorkspaceRuntime` with immutable folder/generation scope, serializable tree state, and folder-owned query prefix; `FileTree` → `FilesApi`; validated library and files HTTP protocols; typed Electron folder release coordination and authoritative reconciliation | First-folder registration and folder removal use additive replacement routes backed by existing server membership and cleanup authority; file listing has an additive strict response contract; legacy folder and file endpoints stay compatible until cutover | Building | Tasks 23–27 prove settled welcome and complete membership states; native open/create/removal and local failure handling; source-preserving confirmation; authoritative active-folder replacement; folder-runtime creation, switching, and generation guards; accessible bounded tree rendering; cross-window release before membership removal; missing-source removal; authoritative notification reconciliation; authorized-scope rebind after a transient unavailable response; and loss/removal retirement with target-only query eviction. Exact removal/loss evidence is listed in [Subphase 2](subphases-2-bootstrap-and-workspace.md#27--handle-folder-loss-and-removal). Session restore remains Task 28; final J02 E2E, accessibility, performance, and visual evidence remains deferred. |
...
| Runtime contract validation | Building | Shared Zod schemas now cover the library folder dialog, uncredentialed renderer runtime configuration, library HTTP request/snapshot/failure envelopes, and the files listing/reveal envelopes. `pnpm test:protocols`, `pnpm test:electron-boundary`, server route/origin tests, renderer adapter tests, and the live `app://` smoke prove these entries; remaining HTTP, IPC, event, WebSocket, worker, and persisted protocols arrive with their owning slices. |
