...
| Documents and editing | J03 | Retain / [0002](../decisions/0002-feature-runtime-and-protocol-ownership.md), [0004](../decisions/0004-runtime-scope-shared-identities-and-navigation.md), [0006](../decisions/0006-rendering-and-resource-discipline.md), [0012](../decisions/0012-persistence-commands-and-contract-evidence.md), [0015](../decisions/0015-adopt-fluid-functionalism-base-ui.md) | Markdown Rendering; Document Viewers; File Transactions; Renderer Workspace | `SourceReference`; folder-scoped `DocumentTabsRuntime`; independently disposable `DocumentRuntime`; app-owned open-document workflow; Fluid document tab surface | Task 28's strict v1 session remains compatible and receives only the bounded active-folder-relative tab projection; source loading and legacy file routes remain unchanged until their owning tasks | Building | Task 29 domain, runtime, composition, file-tree, session, and component evidence listed in [Subphase 3](subphases-3-documents.md) proves folder-plus-path identity, one tab per source, activation, cancellation, child/folder disposal, stale-completion rejection, restored fresh runtimes, bounded persistence, keyboard close, and accessible tabs. Source loading, editing, save/conflict behavior, viewers, and final J03 evidence remain Tasks 30–40 and finalization work. |
...
| Runtime contract validation | Building | Shared Zod schemas now cover the library folder dialog, uncredentialed renderer runtime configuration, library HTTP request/snapshot/failure envelopes, files listing/reveal envelopes, and the bounded v1 workspace-session snapshot plus read/write responses. `pnpm test:protocols`, `pnpm test:electron-boundary`, server route/origin tests, renderer adapter tests, and the live `app://` smoke prove these entries; remaining HTTP, IPC, event, WebSocket, worker, and persisted protocols arrive with their owning slices. |
