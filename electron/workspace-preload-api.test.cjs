...
const {
  createWorkspacePreloadApi,
} = require('../dist/electron/workspace-preload-api.cjs');
...
test('workspace preload validates requests and main-process responses', async () => {
  const ipc = createIpc({ ok: true, folderPath: '/workspace/notes', future: true });
  const api = createWorkspacePreloadApi(ipc);
  ...
  const malformed = createWorkspacePreloadApi(createIpc({ ok: true, folderPath: 42 }));
  ...
  const unavailable = createWorkspacePreloadApi(createIpc(new Error('private detail')));
