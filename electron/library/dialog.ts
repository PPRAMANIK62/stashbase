...
export { LIBRARY_FOLDER_DIALOG_CAPABILITY };
...
const failure = (
  ...
  message: string,
): LibraryFolderDialogFailure =>
  libraryFolderDialogFailureSchema.parse({ ok: false, failure: { kind, message } });
...
export function registerDialog(
  ...
  dependencies.ipcMain.handle(
    LIBRARY_FOLDER_DIALOG_CHANNEL,
    async (event, rawRequest): Promise<LibraryFolderDialogResponse> => {
      ...
      const request = libraryFolderDialogRequestSchema.safeParse(rawRequest);
      if (!request.success) {
      ...
      try {
        ...
        return libraryFolderDialogResponseSchema.parse({
          ok: true,
