...
| Editable-file save version | The file's text bytes | `fileVersion` / `kbRulesVersion` return `sha256:<content>` (`server/files.ts:136`, `server/kb.ts:31`), carried in tab `baseVersion` | Content-only by design: sync providers may rewrite inode/ctime/mtime under `~/Documents`, and metadata churn must not become a save conflict |
...
Autosave conflict guard: editable file saves carry the tab's last-seen content-hash `baseVersion`; the first write attempt rejects if disk content differs (`routes/files.ts:143`, same shape for KB rules), then the renderer treats the active edit buffer as authoritative and retries the same unchanged buffer without `baseVersion` so iCloud/remote sync cannot keep interrupting an active StashBase editing session (`web-src/src/store/AppContext.tsx:933`). This SHA-256 token is separate from the daemon's BLAKE3 indexing hash (§4); it is an HTTP edit-concurrency token, not an index identity.
...
| import-index optimism window | 60s (`importIndexGrace`, `AppContext.tsx`) | After dragging in md/html, light up "stashing" immediately by the real count—the daemon queues `status` behind the embed it's already running, so a bare poll under-counts and lags (4 mds read as "3 stashing"). **Not handed off per path** (mid-embed `pending` is unreliable); `/api/index-status.visibleIndexingSettled` clears it when the UI-visible pending list is empty, while raw `upToDate` keeps the broader daemon consistency meaning (including orphaned/hidden rows) | If there was already a stuck visible pending before import, `visibleIndexingSettled` won't flip → rely on the 60s cap to clear |
