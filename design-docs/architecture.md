...
Markdown is edited, read, and indexed directly. New Markdown tabs open in Live Editing; Reading View is the explicit non-editable surface. The active Markdown tab flushes before entering Reading View and remains in Live Editing on a save error. Each open Markdown tab keeps its CodeMirror instance mounted while hidden, retaining tab-local cursor, selection, scroll, and undo history across Reading View and tab switches. The editor receives a canonical no-BOM/LF text presentation through `server/active-file-operations.ts`, while `server/file-save.ts` restores the source file's UTF-8 BOM presence and uniform newline convention when it writes an edited buffer; raw source syntax stays opaque to that serialization boundary. Renderer callers cross the two-function seam in `web-src/src/markdown.ts`; the document pipeline is implemented under `web-src/src/markdown/` as focused modules for orchestration, sanitization, and preview-document presentation, with package-native Marked extensions for footnotes and heading IDs. The full read-only preview pipeline, iframe boundary, asset resolution, and navigation behavior are described in [markdown-rendering.md](markdown-rendering.md).
