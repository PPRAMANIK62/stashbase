...
- Reporting normally starts from the **Report a bug** row in Settings, under
  General, or from native **Help → Report a Bug…**. The Settings row explains
  itself and stays disabled when the desktop bridge is absent, and the native
  entry remains usable when the main workspace renderer is unhealthy.
