...
A dedicated local review window lets the user describe what happened, what
they expected, and the steps to reproduce. Each available optional artifact
has an explicit two-state include/exclude control whose value comes from the
main process. The inactive alternative remains visible but subdued, while the
selected state is clearly identified as included or excluded. A compact summary
near the final review action repeats the current authoritative choices without
repeating artifact contents. The user can preview an artifact without changing
that value.
...
and availability remain visible as supplementary metadata. The text is
...
action is the internal approval cut line: it freezes the selected configuration
locally and immediately retires deselected or unavailable resources. The main
process then claims that exact snapshot, creates only its selected artifacts in
the current session's temporary report area, and shows a **Report ready**
handoff in the same window. Nothing is uploaded or submitted automatically.
From that handoff, the user may explicitly open the prepared folder followed
by a prefilled GitHub issue, or copy the prepared artifacts to a directory they
choose through the native folder picker.
...
path to the renderer. A user-saved copy is outside that temporary root and is
not affected by next-boot cleanup. The opened GitHub page contains only the
four reviewed report sections, with the approved diagnostics allowlist in the
Environment section when included. Logs, screenshot bytes, internal
identifiers, and filesystem paths are never placed in the URL.
...
| Save selected artifacts     | Cancellation, success, or a structured safe error                                                     | Chosen destination, prepared paths, artifact bytes, or generic filesystem access |
...
draft. Repeated approval returns the same approved summary; an approved or
discarded draft cannot be mutated.
...
* Save and clipboard fallbacks, along with current reveal, browser,
  temporary-file, and cleanup work, stay main-process-owned and extend the
  draft lifecycle rather than adding renderer-side privileged access.
...
* Dedicated review UI with validated user fields, opaque per-draft artifact
  references, exact bounded safe previews, authoritative include/exclude state,
  and explicit local approval.
...
* Session-scoped, selected-only prepared artifacts with final text scans,
  atomic writes, next-boot cleanup, an explicit Report Ready handoff, native
  folder reveal followed by a prefilled browser handoff, and a user-chosen
  durable copy action that never uploads or submits the issue.
