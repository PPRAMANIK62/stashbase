...
Welcome may offer file or folder import only from an explicit Library-folder action. The renderer opens that destination before invoking the existing folder-scoped upload action; it must not introduce a target-free write path or infer a filesystem destination from the source selection.
