...
PDF preview still shows the original PDF, not the derived Markdown. The renderer uses pdf.js against the source PDF, installs the runtime polyfills pdf.js needs in Electron's current Chromium, loads the bundled pdf.js CMaps, fallback fonts, and WASM through same-origin `/pdfjs-assets/*` routes, and asks pdf.js to draw font glyph outlines itself instead of delegating embedded PDF font faces to Chromium. Because search and Agent answers can carry page references from the derived text, `web-src/src/components/PdfPreview.tsx` owns loading, lazy page rendering, retry/status chrome, current-page detection, and page jumping, while `web-src/src/components/pdfText.ts` owns pdfjs text flattening, Markdown-noise cleanup, chunk matching, y-position mapping, highlight rectangle calculation, and pure visible-page selection. At the scroll limit the final page is authoritative because a short final page may be unable to cross the ordinary reading marker. Renderer tab state retains the last viewed page per PDF tab for the session; opening another file in that tab or observing a changed, already-known source version clears the saved page before the viewer reloads. Recording a version for a previously unversioned source is metadata hydration, not a replacement, and must retain the page. Direct page jumps update the tab state synchronously before an immediate tab switch can unmount the viewer. All viewer chrome lives in one row portalled into the main pane's reserved chrome band — zoom out / percentage / zoom in, a Fit-to-width toggle that stays visibly pressed while auto-fit holds, and the current page over the total, which doubles as the jump-to-page field. Fit means the page fills the pane width edge to edge, so the sunken canvas behind the pages shows only between sheets and around a zoomed page; the band above stays on the base surface, like the tab that merges into it, and the viewer's top border is the only thing separating the two — it also gives scrolled content a visible line to be clipped at. The row carries no container of its own, and a fitted page column no top padding, so the document sits at the same height before and after the first scroll; zoomed away from fit the page regains the margin it takes between sheets, because a sheet with room on both sides and none above reads as cropped. Rendered pages carry no page label of their own: at fit-to-width the side gutter is too narrow to hold one, so page position is the chrome row's job.

The chrome band `MainPane` reserves for that row — and for the Markdown/JSON edit toggle, and the empty-tab label — is 32px, and every control that lives in it is 24px. Neither number is a taste call: the band's contents are absolutely positioned at a fixed `top: 44px` while the tab strip above them grows with the interface-size preference (35.7 / 37.6 / 40.4px at the three steps), so 44 is already the highest the chrome can start without the largest strip covering it, and the band must therefore span `44 - 35.7 + 24`. A 28px control in that band hangs into the document below it. `web-src/src/components/MainPane.tsx` dynamically imports the PDF and DOCX viewers so pdfjs and Mammoth-related code stay out of the initial renderer chunk until a user opens those formats.
...
- **Semantic retrieval**: dense vector retrieval, combined with keyword signal through MFS/Milvus.
