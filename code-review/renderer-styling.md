...
1. **Semantic theme variables** (`web-src/src/styles/globals.css` `:root`
   ...
   `--shadow-*` and `--font-*` already use, so the `text-*` utilities and
   colocated CSS all read one definition. `--ui-font-size` is `var(--text-base)` rather
...
5. **Primitives** (`web-src/src/common/components/ui/`) — shadcn adapters
   ...
   **The document `TabStrip` is the third, and it stays hand-rolled — the
   ...
   `scripts/renderer/size.mjs`. It bought nothing: unlike the other
...
| Focused evidence | `web-src/src/common/__tests__/renderer-foundation.test.ts` |
...
surfaces, Appearance Settings, Quick Open, or Command Palette, review the
affected surfaces by eye in the built application across both themes, a
narrow and a wide window, and with reduced motion enabled. Record what was
reviewed with the change.

Run `pnpm typecheck`, `pnpm test:renderer`, and `pnpm build:web` for styling
changes. Pixel-baseline comparison retired with the Playwright suites, so
composition is reviewed by eye against explicit viewport, theme, content, and
reduced-motion conditions. Story accessibility remains the automated floor for
a styling change.
