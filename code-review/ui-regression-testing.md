...
Playwright owns the claims that static analysis cannot decide: one
keyboard-only Quick Open path with focus return, reduced motion on a real
overlay, compact-layout continuity, both application themes, and Linux
screenshots spanning application chrome, workspace navigation, command
surfaces, and a document state.
