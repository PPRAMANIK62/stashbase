...
| State Interfaces | Chat tab state/actions in `web-src/src/store/state/state.ts` and `state/stateReducer.ts`; activation consent in `features/agent-panel/lib/chatActivation.ts`; focused pure state Modules under `features/agent-panel/lib/` |
