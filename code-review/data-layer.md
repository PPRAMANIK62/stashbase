...
JSON is directly readable structured text, including when malformed. The raw
source is the only visible, writable, Agent-readable, and indexable truth. It
never creates AppData-derived text or preparation state, and it remains outside
the Markdown/HTML note vocabulary used for bundles, legacy-derived hiding, and
link cascades. Saving preserves a leading UTF-8 BOM and the source line-ending
convention without parsing, formatting, sorting, or serialization.
The daemon's configured MFS scanner is the single recursive disk walk; it
owns case-insensitive extension admission plus ignore pruning. JSON support
must not add a supplemental traversal that bypasses `.gitignore`, `.mfsignore`,
or excluded-directory pruning.
...
| Search popup query/results | Renderer module memory (`web-src/src/librarySearch.ts`) | No | Is it display-state only, refreshed against current content on reopen, and kept out of the reducer so folder switches cannot clear it? |
| Out-of-folder tab document (`OpenFile.folder`) | Renderer memory | No | Is it strictly read-only (no write path can reach the active folder), does every fetch carry its explicit member folder, and is it exempt from active-folder pruning/remap/delete cascades? |
...
- Direct-text, note-only, convertible, and historical sibling-derived formats are separate vocabularies. The shared catalog owns extension membership and server/renderer matchers derive from it. JSON joins direct text but not notes, bundles, legacy-derived hiding, or link cascades; audio joins conversion/viewing but not compatibility hiding. Private format adapters in the shared dispatch module own queue, discovery, current-output indexing, reset, readiness, and interactive-preparation differences so route/mutation callers do not repeat a format switch.
- Daemon mutation acknowledgements are visibility barriers: source and prefix deletion, including the old side of a vector-reused rename, flush the affected Milvus store before returning. Immediate status and search after a completed mutation must not observe rows that the mutation reported removed.
...
- Background preparation is quiet by default. Collapsed library rows and affected file rows show failure markers only; in-folder headers do not show preparation badges. The search popup is where pending/failed preparation is summarized because that is where incomplete readiness affects the user.
...
- Search is the place that explains how many files are ready and how many are still being prepared — the search popup's readiness banners, which cover the active folder.
