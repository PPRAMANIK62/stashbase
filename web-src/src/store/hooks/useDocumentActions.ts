...
import { buildConflictMarkerDraft } from '@/store/lib/conflictDiff';
import {
  isFolderFileTab,
...
export function useDocumentActions(
  ...
  const scheduleSave = useCallback(() => {
    const tabId = state.current.workspace.activeTabId;
  ...
  const claimConflictResolution = useCallback((tabId: string) => {
    const tab = state.current.workspace.tabs.find((candidate) => candidate.id === tabId);
    if (
      !tab?.conflict
      || !tab.file
  ...
  const resolveConflictOverwrite = useCallback(async (tabId: string) => {
    ...
    const folderPathAtStart = state.current.workspace.folderPath;
