...
test('document tab lifecycle reuses a blank tab and selects a neighbor on close', () => {
  ...
  assert.equal(state.workspace.tabs[0].editMode, true);

  state = reducer(state, { type: 'NEW_TAB' });
  const blankId = state.workspace.activeTabId!;
  state = reducer(state, {
    type: 'FILE_OPEN',
    body: { name: 'two.md', format: 'md', content: 'two' },
  });
  assert.equal(state.workspace.tabs.length, 2);
  assert.equal(state.workspace.activeTabId, blankId);
  assert.equal(state.workspace.tabs[1].file?.name, 'two.md');
...
test('JSON persistent tabs, dirty retention, replacement, and folder isolation use the shared tab lifecycle', () => {
  ...
  assert.equal(state.workspace.tabs[0].file?.name, 'two.JSON');
  assert.equal(state.workspace.tabs[0].editMode, false);
  ...
  assert.equal(state.workspace.tabs.length, 0, 'folder switches cannot retain a JSON editor from another folder');
});

test('document activation maintains folder-local file recency separately from tab order', () => {
  let state = freshState({ workspace: { folderPath: '/folder' } });
  state = reducer(state, { type: 'FILE_OPEN', body: { name: 'one.md', format: 'md', content: '' } });
  const oneId = state.workspace.activeTabId!;
  state = reducer(state, { type: 'NEW_TAB' });
  state = reducer(state, { type: 'FILE_OPEN', body: { name: 'two.md', format: 'md', content: '' } });
  assert.deepEqual(state.workspace.recentFilePaths, ['two.md', 'one.md']);

  state = reducer(state, { type: 'ACTIVATE_TAB', id: oneId });
  assert.deepEqual(state.workspace.recentFilePaths, ['one.md', 'two.md']);

  state = reducer(state, {
    type: 'FILES_LOADED', files: [], folders: [], folder: 'other', folderPath: '/other',
  });
  assert.deepEqual(state.workspace.recentFilePaths, []);
});
...
test('Editor History tracks tab-id MRU order independent of tab-strip order and prunes closed tabs', () => {
  ...
  assert.deepEqual(state.workspace.editorHistory, [blankId, oneId]);
  ...
  state = reducer(state, { type: 'ACTIVATE_TAB', id: oneId });
  assert.deepEqual(state.workspace.editorHistory, [oneId, blankId]);

  // Closing a tab drops it from history even though it isn't the most
  // recent entry.
  state = reducer(state, { type: 'CLOSE_TAB', id: blankId });
  assert.deepEqual(state.workspace.editorHistory, [oneId]);

  state = reducer(state, {
    type: 'FILES_LOADED', files: [], folders: [], folder: 'other', folderPath: '/other',
  });
  assert.deepEqual(state.workspace.editorHistory, []);
});
...
test('Editor History survives sidebar pruning of missing file tabs', () => {
  ...
  const keepId = state.workspace.activeTabId!;
  state = reducer(state, { type: 'NEW_TAB' });
  state = reducer(state, {
    type: 'FILE_OPEN',
    body: { name: 'gone.md', format: 'md', content: '' },
  });
  assert.equal(state.workspace.editorHistory.length, 2);

  state = reducer(state, { type: 'PRUNE_MISSING_FILE_TABS', names: ['keep.md'] });
  assert.deepEqual(state.workspace.editorHistory, [keepId]);
});
...
test('a save acknowledgement advances the version without replacing the live document source', () => {
  ...
  assert.equal(next.workspace.tabs[0].file?.content, '```ts\nconst live = true\n```');
  assert.equal(next.workspace.tabs[0].file?.version, 'after');
});
...
test('replacing a PDF source version clears its saved page', () => {
  ...
  assert.equal(unchanged.workspace.tabs[0].pdfPage, 10);

  const replaced = reducer(state, { type: 'FILE_PATCH', patch: { version: 'after' } });
  assert.equal(replaced.workspace.tabs[0].file?.version, 'after');
  assert.equal(replaced.workspace.tabs[0].pdfPage, undefined);
});
...
test('only dirty missing document tabs survive sidebar pruning', () => {
  ...
  state = reducer(state, { type: 'DOCUMENT_DIRTY', dirty: true });
  state = reducer(state, { type: 'PRUNE_MISSING_FILE_TABS', names: [] });
  assert.equal(state.workspace.tabs[0].file?.name, 'draft.md');
});
...
test('folder path remap updates files, tabs, expansion, focus, and manual order together', () => {
  ...
  assert.deepEqual(next.workspace.files.map((file) => file.name), ['archive/a.md']);
  assert.deepEqual(next.workspace.folders.map((folder) => folder.path), ['archive', 'archive/sub']);
  assert.equal(next.workspace.tabs[0].file?.name, 'archive/a.md');
  assert.deepEqual(next.workspace.recentFilePaths, ['archive/a.md']);
  assert.deepEqual(Object.keys(next.workspace.expanded), ['archive', 'archive/sub']);
  assert.equal(next.workspace.activeFolder, 'archive/sub');
  assert.equal(next.workspace.selectedPath, 'archive/a.md');
  assert.deepEqual(next.workspace.fileOrder, {
    '': ['archive'],
    archive: ['a.md', 'sub'],
    'archive/sub': ['b.md'],
  });
...
test('chat tab recency survives panel toggles and is cleaned as tabs close', () => {
  ...
  assert.deepEqual(state.chat.chatTabRecencyByAgent.codex, [second.id, first.id]);
  state = reducer(state, { type: 'CHAT_TOGGLE' });
  assert.equal(state.chat.chatOpen, false);
  ...
  assert.equal(state.chat.chatOpen, true);
  assert.equal(state.chat.activeChatTabId, first.id);

  state = reducer(state, { type: 'CHAT_TAB_CLOSE', id: first.id });
  assert.equal(state.chat.activeChatTabId, second.id);
  assert.deepEqual(state.chat.chatTabRecencyByAgent.codex, [second.id]);
  state = reducer(state, { type: 'CHAT_TAB_CLOSE', id: second.id });
  assert.equal(state.chat.activeChatTabId, null);
  assert.equal(state.chat.chatOpen, false);
  assert.deepEqual(state.chat.chatTabRecencyByAgent, {});
});
...
test('CHAT_TAB_SET_AGENT switches only blank tabs and migrates recency', () => {
  ...
  assert.equal(state.chat.chatTabs[0].agent, 'claude');
  assert.equal(state.chat.chatTabs[0].title, 'New Chat');
  assert.deepEqual(state.chat.chatTabRecencyByAgent.codex, [started.id]);
  assert.deepEqual(state.chat.chatTabRecencyByAgent.claude, [blank.id]);
  ...
  assert.equal(state.chat.chatTabs[1].agent, 'codex');
...
test('CHAT_TAB_SET_AGENT renumbers the placeholder title for the new agent', () => {
  ...
  let state = freshState({ chat: { chatTabs: [claudeTab, blank], activeChatTabId: blank.id } });
  state = reducer(state, { type: 'CHAT_TAB_SET_AGENT', id: blank.id, agent: 'claude' });
  // A second claude tab picks up the "New Chat 2" numbering, matching
  // makeChatTab's per-agent placeholder scheme.
  assert.equal(state.chat.chatTabs[1].agent, 'claude');
  assert.equal(state.chat.chatTabs[1].title, 'New Chat 2');
});
...
test('new chat tabs start blank and AgentView updates keep the flag current', () => {
  ...
  assert.equal(state.chat.chatTabs[0].blank, false);
...
test('pendingResume channel: request replaces and consume clears', () => {
  ...
  assert.deepEqual(state.chat.pendingResume, { agent: 'claude', sessionId: 's1', folder: '/lib/notes' });
  ...
  assert.deepEqual(state.chat.pendingResume, { agent: 'codex', sessionId: 's2', folder: null });
...
test('out-of-folder tabs stay read-only and outside folder-local flows', () => {
  let state = reducer(freshState({ workspace: { folder: 'A', folderPath: '/a' } }), {
    type: 'FILE_OPEN',
    body: { name: 'notes/远方.md', format: 'md', content: '# hi' },
    libraryFolder: '/b',
  });
  const tab = state.workspace.tabs[state.workspace.tabs.length - 1];
  ...
  state = reducer(state, { type: 'PRUNE_MISSING_FILE_TABS', names: [] });
  assert.ok(state.workspace.tabs.some((t) => t.id === tab.id));
  ...
  assert.equal(state.workspace.tabs.find((t) => t.id === tab.id)?.file?.name, 'notes/远方.md');
...
test('PDF page numbers are isolated per tab and reset when replacing a file', () => {
  ...
  const tabId = state.workspace.activeTabId!;
  assert.equal(state.workspace.tabs[0].pdfPage, undefined);

  // Set the PDF page
  state = reducer(state, { type: 'TAB_PDF_PAGE', id: tabId, page: 4 });
  assert.equal(state.workspace.tabs[0].pdfPage, 4);
  ...
  const secondTabId = state.workspace.activeTabId!;
  assert.equal(state.workspace.tabs.length, 2);
  assert.equal(state.workspace.tabs[0].pdfPage, 4); // retained on the first tab
  assert.equal(state.workspace.tabs[1].pdfPage, undefined); // undefined on the second tab

  state = reducer(state, { type: 'TAB_PDF_PAGE', id: secondTabId, page: 2 });
  state = reducer(state, { type: 'ACTIVATE_TAB', id: tabId });
  assert.equal(state.workspace.tabs.find((tab) => tab.id === tabId)?.pdfPage, 4);
  state = reducer(state, { type: 'ACTIVATE_TAB', id: secondTabId });
  assert.equal(state.workspace.tabs.find((tab) => tab.id === secondTabId)?.pdfPage, 2);
