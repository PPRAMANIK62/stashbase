...
import { initialState, makeTab, reducer, type Action, type State, type Tab } from '@/store/state/state';
...
test('mounted JSON uses production dirty, debounce, conflict, in-flight, and close-save actions', async () => {
  ...
  try {
    ...
    assert.ok(control.current!.state.current.workspace.tabs[0].conflict, 'transitions to conflict state');
    assert.equal(control.current!.state.current.workspace.tabs[0].conflict?.diskContent, '{"disk": true}');
    assert.match(
      control.current!.state.current.workspace.tabs[0].conflict?.editorContent ?? '',
      new RegExp(lateConflictEdit),
      'the conflict snapshot includes edits typed while the stale save was in flight',
    );
    assert.equal(
      control.current!.state.current.workspace.tabs[0].conflict?.diskVersion,
      'version-disk',
    ...
    await act(async () => {
      await control.current!.actions.resolveConflictReload(control.current!.state.current.workspace.activeTabId!);
      await Promise.resolve();
    });
...
test('conflict resolution callbacks apply correct edits and state transitions', async () => {
  ...
  try {
    ...
    const tabId = control.current!.state.current.workspace.activeTabId!;
    ...
    // Verify setup
    assert.ok(control.current!.state.current.workspace.tabs[0].conflict);

    // 2. Test reload
    await act(async () => {
      await control.current!.actions.resolveConflictReload(tabId);
    });
    assert.equal(control.current!.state.current.workspace.tabs[0].conflict, null);
    assert.equal(control.current!.state.current.workspace.tabs[0].dirty, false);
    assert.equal(control.current!.state.current.workspace.tabs[0].file?.content, '{"disk": true}');
    assert.equal(control.current!.state.current.workspace.tabs[0].file?.version, 'v-disk');
    ...
    const resolvingConflict = control.current!.state.current.workspace.tabs[0].conflict as Tab['conflict'];
    ...
    assert.ok(control.current!.state.current.workspace.tabs[0].conflict, 'the first decision owns the unresolved UI');
    assert.equal(control.current!.state.current.workspace.tabs.length, 1, 'tab discard cannot overtake the owning decision');
    ...
    assert.equal(control.current!.state.current.workspace.tabs[0].conflict, null);
    assert.equal(control.current!.state.current.workspace.tabs[0].dirty, false);
    assert.equal(control.current!.state.current.workspace.tabs[0].file?.content, '{"editor": true}');
    assert.equal(control.current!.state.current.workspace.tabs[0].file?.version, 'overwritten-version');
    ...
    const failedConflict = control.current!.state.current.workspace.tabs[0].conflict as Tab['conflict'];
    ...
    assert.equal(control.current!.state.current.workspace.tabs[0].conflict, null, 'a later resolution can claim the conflict');
    assert.equal(control.current!.state.current.workspace.tabs[0].file?.content, '{"diskAfterFailure": true}');
    ...
    assert.equal(control.current!.state.current.workspace.tabs[0].conflict, null);
    assert.equal(control.current!.state.current.workspace.tabs[0].dirty, true, 'merged state remains dirty');
    const content: string = control.current!.state.current.workspace.tabs[0].file?.content ?? '';
