 ...
 * ARIA state and the visual state can never drift apart.
 */

export const PICKER_VEIL_CLASS =
  'fixed inset-0 z-1200 flex items-start justify-center bg-black/18 pt-[min(18vh,150px)]';
...
export const PICKER_ROW_CLASS =
  'group flex cursor-default justify-between gap-5 rounded-sm px-2.5 py-1.75 aria-selected:bg-accent aria-selected:text-primary-foreground';

/** The muted right-hand detail (`<small>`) inside a result row. */
export const PICKER_ROW_DETAIL_CLASS =
  'overflow-hidden text-ellipsis whitespace-nowrap text-muted-foreground group-aria-selected:text-inherit group-aria-selected:opacity-80';

export const PICKER_EMPTY_ROW_CLASS =
  'flex cursor-default justify-center px-2.5 py-4.5 text-muted-foreground';
