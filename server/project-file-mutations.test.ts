...
test('MCP project mutations work outside an active folder and enforce versions', async (t) => {
  const testHome = fs.mkdtempSync(path.join(os.tmpdir(), 'stashbase-project-mutations-'));
...
$$`;
  ...
  for (const replaceAll of [false, true]) {
    ...
    const initial = replaceAll ? 'before old old after' : 'before old after';
    const written = await callTool(base, token, 'write_file', { path: literalPath, content: initial });
    const editedLiteral = await callTool(base, token, 'edit_file', {
      path: literalPath, old_text: 'old', new_text: literalReplacement,
      replace_all: replaceAll, baseVersion: written.version,
    });
    assert.ok(editedLiteral.version);
    assert.equal(fs.readFileSync(literalPath, 'utf8'), replaceAll
      ? `before ${literalReplacement} ${literalReplacement} after`
      : `before ${literalReplacement} after`);
  }
  ...
  const workbenchMoved = await projectMutations.moveProjectFile(opaqueSource, opaqueTarget, { allowOpaque: true });
  ...
  const workbenchDeleted = await projectMutations.deleteProjectFile(opaqueTarget, { allowOpaque: true });
  ...
  const movedHiddenText = await projectMutations.moveProjectFile(hiddenTextSource, hiddenTextTarget);
  ...
  const movedHiddenAudio = await projectMutations.moveProjectFile(hiddenAudioSource, hiddenAudioTarget);
