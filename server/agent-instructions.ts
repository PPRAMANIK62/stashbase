 ...
 * preserve those bytes when composing their separate internal policy. The default lives in `default.md`. */
...
export function createAgentInstructionsStore(io: {
  ...
  function get(scope: AgentInstructionsScope): AgentInstructionsState {
    ...
    const text = readableText(storedFolders(config).find((entry) => pathsEqual(entry.path, scope.path))?.text);
  ...
  function set(scope: AgentInstructionsScope, rawText: string): AgentInstructionsState {
    ...
    const folders = storedFolders(config).filter((entry) => !pathsEqual(entry.path, scope.path));
    if (text) folders.push({ path: scope.path, text });
...
export function resolveAgentInstructions(folderPath: string): string {
  return store.get({ kind: 'folder', path: folderPath }).text;
}
